...
export default function AuthForm() {
  ...
  const handleSignup = async () => {
    const { data: signupData } = await axios.post('/api/user/sign-up', formData);
    await handleSignin(); 
  };
  ...
  // If logged in, show Connect Wallet + Logout
  if (isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black p-6">
        <Navigation />
        <div className="text-center space-y-4">
          <h2 className="text-3xl font-bold text-white">Welcome, {localStorage.getItem('role')}</h2>
          <button className="py-3 px-6 bg-green-600 hover:bg-green-700 text-white rounded-lg transition">
            Connect Wallet
          </button>
          <button
              ...
              <>
                ...
                <div>
                  ...
                  <select
                    ...
                    value={formData.role}
                    onChange={handleChange}
                    className="w-full px-4 py-2 rounded-lg bg-gray-900/80 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    required
                  >
